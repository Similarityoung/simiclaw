...
export function ConsoleRuntime({ runs, selectedRunID, trace, loadingTrace, onSelectRun }: ConsoleRuntimeProps): JSX.Element {
  return (
    <Card className="min-h-[320px]">
      ...
      <CardContent className="grid gap-4 lg:grid-cols-[minmax(0,240px)_minmax(0,1fr)]">
          ...
          {runs.length === 0 ? (
            ...
            runs.map((run) => (
              ...
              >
                ...
                <div className="mt-1 text-xs opacity-80">{run.run_id}</div>
              </button>
            ))
          )}
        </div>
