...
- `internal/bootstrap/`: composition root 与进程生命周期；只负责 wiring，不拥有四块中的业务 owner
- `internal/gateway/`: Runtime 写路径入口、binding/routing、校验、限流、幂等与 session scope 解析
- `internal/runtime/` + `internal/runner/` + `internal/outbound/`: Runtime kernel、EventLoop、payload/worker/lane owner、LLM 执行、runtime observe 与 durable delivery 协调
- `internal/store/`: SQLite 初始化、schema，以及 `tx` / `projections` / `queries` 三层事实与读写职责
- `internal/query/` + `internal/http/`: query read model 与 HTTP surface adapters（ingest/query/stream/readiness）
- `internal/prompt/` + `internal/memory/`: prompt 组装、memory 与上下文资产
- `internal/provider/` + `internal/tools/`: Capability Plane，可调用 provider/tool source，不直接推进 durable state
