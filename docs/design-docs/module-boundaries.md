...
| 规则 | 说明 | 证据 |
| --- | --- | --- |
| 非存储层不得直接依赖 `internal/store` | `httpapi`、`ingest`、`channels`、`workspace`、`query`、`runner`、`runtime` 都被禁止直接 import store | `TestHTTPAPIProductionCodeDoesNotImportStore` 等 |
...
- HTTP 路由: `internal/httpapi/routes.go`
