...
| 提交后发送与流式通知 | `internal/outbound`, `internal/streaming`, `internal/http/stream`, `internal/query`, `internal/store/queries` | durable delivery、runtime event stream、SSE、read DTO | 真实出站发送只能晚于 outbox 持久化；HTTP stream 只消费 runtime events |
