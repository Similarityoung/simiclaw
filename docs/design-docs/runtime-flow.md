...
SimiClaw 的主链路是一个典型的“先持久化、后执行、再结果提交”的两阶段 runtime：event 先进入 SQLite，再由 EventLoop 调度 event work、按 lane 串行化，最终交给 kernel 执行 claim -> execute -> finalize。
...
`spec03` 的最终收口要求是：composition root 只拼装 seam，不回流拥有行为。因此 `internal/bootstrap/app.go` 会先装配 capability/context-state/runtime，再把这些 seam 交给 HTTP/CLI/Telegram surface；`internal/http/server.go` 只注册 route，不承接 runtime orchestration 或 client fallback。
...
- `EventLoop` 同时消费内存队列和 SQLite 中的 runnable event，并以 event work 作为唯一调度单元。
...
- Runtime 与 Runner 的边界固定为两类 adapter：`runtime.NewRunnerExecutor` 负责 claim -> runner invocation -> finalize result translation，`runtimeEventStreamSink` 负责 delta/tool event translation + metadata publish，不让这些 glue 回流到 EventLoop 或 HTTP surface。
